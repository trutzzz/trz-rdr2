...
AddEventHandler('goldpanner:StartPaning', function()
    if not Panning then 
        ...
        for k,v in pairs(WaterTypes) do
            if Water == WaterTypes[k]["waterhash"]  then
                ...
                for i=1,seconds,1 do
                    TriggerEvent("vorp:Tip", Config.info1, 500)
                    Wait(1350)
                    TriggerEvent("vorp:Tip", Config.info2, 500)
                    Wait(1350) 
                    TriggerEvent("vorp:Tip", Config.info3, 500)
                    Wait(1350) 
                end
                -- Wait(w)
                ClearPedTasks(ped)
        ...
        if foundwater == false then
            TriggerEvent("vorp:TipBottom", Config.no_akses, 10000)
        end
    end
end)
