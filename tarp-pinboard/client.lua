				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.coords[1], v.coords[2], v.coords[3], false) < 1.0 then 
					...
					if active == false then
						if Citizen.InvokeNative(0xC92AC953F0A982AE,menubutton) then
							city = v.city
							TriggerServerEvent("tarp-pinboard:get_posters", v.city)
							active = true
							Citizen.Wait(1000)
							Show_Poster(true)
						end
...
RegisterNetEvent('tarp-pinboard:send_list')
AddEventHandler('tarp-pinboard:send_list', function(table,x,y)
	poster_table = table
	charidentifier = x
	if y == "admin" then 
		group = true 
    ...
    for k,v in pairs(Config.Posters) do
		...
		if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.coords[1], v.coords[2], v.coords[3], false) < 10.0 then
			if data.Title ~= "" then
				...
				TriggerServerEvent("tarp-pinboard:set_link", data.Title, s:gsub("%'", ""), v.city)				
				Citizen.Wait(1000)
				TriggerServerEvent("tarp-pinboard:get_posters", v.city)
...
RegisterNUICallback('removepin', function(data, cb)
	...
	active = false
	cb('ok')
	TriggerServerEvent("tarp-pinboard:removeposter", data.id)
end)
