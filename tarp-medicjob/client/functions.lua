...
function RevivePlayer(playerPed)
    ...
    if IsPedDeadOrDying(closestPlayerPed, 1) then
        ...
        exports['tarp-notify']:SendAlert('inform', _U('revive_in_prog'))
        Wait(2000)
        ClearPedTasksImmediately(PlayerPedId())
        TriggerServerEvent('rwt-medic:reviveplayer', GetPlayerServerId(playerPed))
        WarMenu.CloseMenu()
    else
        --TriggerEvent("vorp:TipBottom", _U('player_not_unconscious'), 3000)
        exports['tarp-notify']:SendAlert('inform', _U('player_not_unconscious'))
    end
end
...
function HealPlayer(playerPed)
    ...
    exports['tarp-notify']:SendAlert('inform', _U('heal_in_prog'))
    Wait(10000)
    ClearPedTasksImmediately(PlayerPedId())
    TriggerServerEvent('rwt-medic:healplayer', GetPlayerServerId(playerPed))
    WarMenu.CloseMenu()
end
