...
function startPlay()
    resetGameVariables()
    inTable = true
    TriggerServerEvent('tarp-blackjack:Request_Sit' , target_table)
...
function stopPlay()
    resetGameVariables()
    inTable = false
    TriggerServerEvent('tarp-blackjack:Request_Leave', target_table)
        ...
        while distance do
            ...
            if not inTable then
                ...
                if PromptHasStandardModeCompleted(_StartPrompt) then
                    print("sdsds")
                    TriggerEvent('vorp:TipBottom', 'When playing BlackJack you must not exceed a total of 21. The picks count for 10 and the aces for 10 or 1. Whoever comes closest to 21 wins.' , 10000 )
            ...
            if inTable and waiting and not alreadyPressed then
                ...
                if PromptHasStandardModeCompleted(_CallPrompt) and waiting then
                    TriggerServerEvent('tarp-blackjack:Choice', true , i)
                    alreadyPressed = true
                    PromptSetEnabled(_CallPrompt, false)
                    PromptSetVisible(_CallPrompt, false)
                end
            ...
            if PromptHasStandardModeCompleted(_BetPrompt) and not stopBet then
                if not stopBet then
                    TriggerServerEvent("tarp-blackjack:Get_Bet", bet , i)
...
RegisterNetEvent('tarp-blackjack:ReceiveCard')
AddEventHandler('tarp-blackjack:ReceiveCard', function(card, type , target)
    cardsGiven = true
	if inTable then
...
RegisterNetEvent('tarp-blackjack:ReceiveWin')
AddEventHandler('tarp-blackjack:ReceiveWin', function(gameResolution)
    textWinLosePush = gameResolution
    PlaySoundFrontend("show_info", "Study_Sounds", true, 0)
    --TriggerEvent( 'ak_notification:Tip', textWinLosePush..' !' , 3000 )
    exports['tarp-notify']:SendAlert('inform', ''..textWinLosePush..' !')
end)

RegisterNetEvent('tarp-blackjack:ReceiveCountedCards')
AddEventHandler('tarp-blackjack:ReceiveCountedCards', function(count,type)
    if type == "player" then
        playerCount=count
    else
        dealerCount=count
...
RegisterNetEvent('tarp-blackjack:GameFinished')
AddEventHandler('tarp-blackjack:GameFinished', function(status)
    gameFinished = status
end)

RegisterNetEvent('tarp-blackjack:WaitingForResponse')
AddEventHandler('tarp-blackjack:WaitingForResponse', function(status)
    alreadyPressed = false
    waiting = status
end)

RegisterNetEvent('tarp-blackjack:Reset_Game_Var')
AddEventHandler('tarp-blackjack:Reset_Game_Var', function()
    resetGameVariables()
end)

RegisterNetEvent('tarp-blackjack:Bet')
AddEventHandler('tarp-blackjack:Bet', function(max)
    maxbet = max


end)

RegisterNetEvent("tarp-blackjack:Stop_Bet")
AddEventHandler("tarp-blackjack:Stop_Bet", function()
    if not stopBet then
        stopBet = true
        PromptSetEnabled(_BetPromptAmmount, false)
        PromptSetVisible(_BetPromptAmmount, false)
        PromptSetEnabled(_BetPrompt, false)
        PromptSetVisible(_BetPrompt, false)
        TriggerServerEvent("tarp-blackjack:Get_Bet", 0 , target_table)
    end
end)
