...
function collectBet(index)
    for i, v in ipairs(playersData[index]) do
        TriggerClientEvent("tarp-blackjack:Bet", v.playerId,25)
    end
    Citizen.Wait(15000)
    for i, v in ipairs(playersData[index]) do
        TriggerClientEvent("tarp-blackjack:Stop_Bet", v.playerId)
    end
end
function ReciveForEveryoneFromSession(card , type , id , index)
	for i, v in ipairs(playersData[index]) do
		TriggerClientEvent('tarp-blackjack:ReceiveCard', v.playerId, card, type ,id)
	end
end
function giveCards(index) -- Questi valori vanno passati al client
    ...
    for i, v in ipairs(playersData[index]) do
        ...
        Citizen.Wait(1000)
        TriggerClientEvent('tarp-blackjack:ReceiveCard', v.playerId, dealerCards[index][1], "dealer")
        Citizen.Wait(1000)
        TriggerClientEvent('tarp-blackjack:ReceiveCountedCards', v.playerId, v.countPlayerCards, "player")
        TriggerClientEvent('tarp-blackjack:ReceiveCountedCards', v.playerId, countDealerCards[index], "dealer")
        ...
        if (#playersData[index] ~= 0) then
            ...
            for i, v in ipairs(playersData[index]) do
                while v.countPlayerCards < 21 and not v.continue do
                    if (v.isInTable) then
                        TriggerClientEvent('tarp-blackjack:WaitingForResponse', v.playerId, true)
                        Citizen.Wait(7000)
                        TriggerClientEvent('tarp-blackjack:WaitingForResponse', v.playerId, false)
                    end
            ...
            for i, v in ipairs(playersData[index]) do
                if (v.isInTable) then
                    TriggerClientEvent('tarp-blackjack:ReceiveCard', v.playerId, dealerCards[index][2], "dealer")
                    Citizen.Wait(1000)
                    TriggerClientEvent('tarp-blackjack:ReceiveCountedCards', v.playerId, countDealerCards[index], "dealer")
                end
            end
           ...
           local j = 3
            while countDealerCards[index] < 17 do
                ...
                for i, v in ipairs(playersData[index]) do
                    if (v.isInTable) then
                        TriggerClientEvent('tarp-blackjack:ReceiveCard', v.playerId, dealerCards[index][j], "dealer")
                        TriggerClientEvent('tarp-blackjack:ReceiveCountedCards', v.playerId, countDealerCards[index], "dealer")
                        Citizen.Wait(1000)
                    end
                end
            ...
            for i, v in ipairs(playersData[index]) do
                ...
                if (v.isInTable) then
                    if countDealerCards[index] > 21 and v.isInTable and v.countPlayerCards > 21 then
                     --   TriggerClientEvent('tarp-blackjack:ReceiveWin', v.playerId, "")
                    ...
                    TriggerClientEvent('tarp-blackjack:GameFinished', v.playerId, true)
                else
   ...
   local _source = source
    ...
    local money2 = money*2
		if condition == "lose" then
            ...
            TriggerClientEvent('tarp-notify:client:SendAlert', playerId, { type = 'inform', text = "Dealer won you lost $" ..money.. "" })
		elseif condition == "win" then
            ...
            TriggerClientEvent('tarp-notify:client:SendAlert', playerId, { type = 'inform', text = "You Won $" ..money2.. "" })
		...
		elseif condition == "push" then
           ...
           TriggerClientEvent('tarp-notify:client:SendAlert', playerId, { type = 'inform', text = "Push! Take your Money Back" })
            Character.addCurrency(0,  money2)
...
function resetGame(index)
    ...
    for i, v in ipairs(playersData[index]) do
        ...
        v.betAmount = 0
        TriggerClientEvent('tarp-blackjack:Reset_Game_Var', v.playerId)
    end
end
...
RegisterNetEvent('tarp-blackjack:Choice')
AddEventHandler('tarp-blackjack:Choice', function(choice , index)
    for i, v in ipairs(playersData[index]) do
...
RegisterNetEvent('tarp-blackjack:Request_Sit')
AddEventHandler('tarp-blackjack:Request_Sit', function(index)
...
RegisterNetEvent('tarp-blackjack:Request_Leave')
AddEventHandler('tarp-blackjack:Request_Leave', function(index)
    for i, v in ipairs(playersData[index]) do
        if v.playerId == source then
            v.isInTable = false
        end
...
RegisterNetEvent('tarp-blackjack:Get_Bet')
AddEventHandler('tarp-blackjack:Get_Bet', function(bet , index)
if index ~= nil then
    for i, v in ipairs(playersData[index]) do
        if v.playerId == source then
